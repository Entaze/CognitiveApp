...
function Cognitivetest () {
 ...
 useEffect(()=>{
   if (userId) {
    ...
    .then((res)=>{
      // console.log('Test1Tracker :', res.data.user.Test1Tracker)
      //Set test trials
      setTestListATrials(res.data.user.Test1Tracker)
      if (res.data.user.Test1Tracker < 6) {
        setBeginSec1(true)
        setFirstPage(true)
        const xTimes = 6 - res.data.user.Test1Tracker;
        setTimes(xTimes)
        // setIntroStep(true)
      }
    })
