...
const Cognitivetest4 = () => {
 ...
 useEffect(()=>{
  if (userAuth) {
    if (!userProfile.test4CompletionSctn1) {
      setTestBegin(true);
      setFirstPage(true);
    } else if (!userProfile.test4CompletionSctn2) {
      setTestBegin2(true)
      setSeventhPage(true);
    } else if (!userProfile.test4CompletionSctn3) {
      setTestBegin3(true)
      setTenthPage(true);
    } else if (!userProfile.test4CompletionSctn4) {
      setMainTestBegin1(true);
      setThirteenthPage(true);
    } else if (!userProfile.test4CompletionSctn5) {
      setMainTestBegin1(true);
      setFifteenthPage(true);
    } else if (!userProfile.test4CompletionSctn6) {
      setMainTestBegin1(true);
  ...
  useEffect(()=>{
    ...
    if (AxiosSend && plusSign && (count <= test1Letter.length)) {
        ...
        //  console.log('Param :', param)
          axios.post('/api/test4section1', param)
          .then((res)=>{
            // console.log('Res data :', res.data.Test4Section1)
            if (count === test1Letter.length) {
              let complete = {_id: userProfile._id, test4CompletionSctn1: true};
  ...
  useEffect(()=>{
    ...
    if (AxiosSend0 && plusSign && (count0 <= test01Letter.length)) {
        ...
        const param = {_id: userProfile._id, obj};
        if (count0 > 0) {
          axios.post('/api/test4section4', param)
          .then((res)=>{
            // setLttr(null)
            if (count0 === test01Letter.length) {
              let complete = {_id: userProfile._id, test4CompletionSctn4: true};
              axios.post('/api/user', complete)
              .then((res) => {
                setTimeout(()=>{
                  setTestBegin1(false)
  ...
  useEffect(()=>{
    ...
    if (AxiosSend2 && plusSign && (count1 <= test2Letter.length)) {
        ...
        const param = {_id: userProfile._id, obj};
        if (count1 > 0 && lttr) {
          axios.post('/api/test4section2', param)
          .then((res)=>{
            // setLttr(null)
            if (count1 === test2Letter.length) {
              let complete = {_id: userProfile._id, test4CompletionSctn2: true};
              // console.log('Complete :', complete)
              axios.post('/api/user', complete)
              .then((res) => {
                setTimeout(()=>{
  ...
  useEffect(()=>{
    ...
    if (AxiosSend3 && plusSign && (count2 <= test3Letter.length)) {
        ...
        const param = {_id: userProfile._id, obj};
        if (count2 > 0) {
          axios.post('/api/test4section3', param)
          .then((res)=>{
            // console.log('Res data :', res.data.Test4Section3)
            if (count2 === test3Letter.length) {
              let complete = {_id: userProfile._id, test4CompletionSctn3: true};
              axios.post('/api/user', complete)
              .then((res) => {
                setTimeout(()=>{
                  setMainTestBegin1(true)
  ...
  useEffect(()=>{
    ...
    if (AxiosSend03 && plusSign && (count02 <= test03Letter.length)) {
        ...
        const param = {_id: userProfile._id, obj};
        if (count02 > 0 && lttr) {
          axios.post('/api/test4section6', param)
          .then((res)=>{
            if (count02 === test03Letter.length) {
              const now = moment();
              const timeC = now._d;
              let complete = {_id: userProfile._id, test4CompletionSctn6: true, Sctn6CompletionTime: timeC };
              axios.post('/api/user', complete)
              .then((res) => {
                setTimeout(()=>{
                  setTestBegin1(false)
