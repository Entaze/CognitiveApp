...
function CognitiveTest4Recallv2 () {
...
useEffect(()=>{
  if (userAuth) {
    if (!userProfile.test4CompletionRecallSctn1v2) {
      setMainTestBegin1(true);
      setFourteenthPage(true);
    } else if (!userProfile.test4CompletionRecallSctn2v2) {
      setMainTestBegin1(true)
      setFifteenthPage(true);
    } else if (!userProfile.test4CompletionRecallSctn3v2) {
      setMainTestBegin1(true)
      setSixteenthPage(true);
    } else {
      // console.log('Hee :', userProfile.test4CompletionRecallSctn1v2)
      setMainTestBegin1(true);
      setTest8End(true)
    }
  }
...
useEffect(()=>{
  ...
  if (AxiosSend0 && plusSign && (count0 <= test01Letter.length)) {
      ...
      if (count0 > 0) {
        axios.post('/api/test4recallsection1v2', param)
        .then((res)=>{
          // console.log('Res data :', res.data.Test4RecallSection1)
        // setLttr(null)
          if (count0 === test01Letter.length) {
            let complete = {_id: userProfile._id, test4CompletionRecallSctn1v2: true};
...
useEffect(()=>{
  ...
  if (AxiosSend02 && plusSign && count01 <= test02Letter.length) {
      ...
      const param = {_id: userProfile._id, obj};
      if (count01 > 0) {
        axios.post('/api/test4recallsection2v2', param)
        .then((res)=>{
          // console.log('Res data :', res.data.Test4Section1)
          if (count01 === test02Letter.length) {
            let complete = {_id: userProfile._id, test4CompletionRecallSctn2v2: true};
            // console.log('Complete :', complete)
            axios.post('/api/user', complete)
            .then((res) => {
              setTimeout(()=>{
...
useEffect(()=>{
  ...
  if (AxiosSend03 && plusSign && count02 <= test03Letter.length) {
      ...
      const param = {_id: userProfile._id, obj};
      if (count02 > 0) {
        axios.post('/api/test4recallsection3v2', param)
        .then((res)=>{
          if (count02 === test03Letter.length) {
            const now = moment();
            const timeC = now._d;
            let complete = {_id: userProfile._id, test4CompletionRecallSctn3v2: true, Test4RecallCompletionTimev2: timeC };
            axios.post('/api/user', complete)
            .then((res) => {
              setTimeout(()=>{
                setTestBegin1(false)
...
const handleSecondSessionEnd = () => {
  let complete = { _id: userId, test4CompletionRecallv2: true, email: userProfile.email, name: userProfile.name }
  axios.post('/api/user', complete)
  .then((res)=>{
    navigate('/test-end')
  })
...
export default CognitiveTest4Recallv2
